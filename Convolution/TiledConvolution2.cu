...
#define Needed_WIDTH (O_TILE_WIDTH + Mask_width - 1)
...
__global__ void TiledConvolution_2D(unsigned char * InputImage, unsigned char * OutputImage, const float *__restrict__ M, int numRows, int numCols, int Channels)
{
	...
	__shared__ float Ns[Needed_WIDTH][Needed_WIDTH][3];
	for (int py = 0; py < 2; py++)
	{
		for (int px = 0; px < 2; px++)
		{
			if((ty + py*O_TILE_WIDTH < Needed_WIDTH) && (tx + px*O_TILE_WIDTH < Needed_WIDTH))//check if the threads in the second phase are needed to copy
			{
				if (((row_i + py*O_TILE_WIDTH) >= 0) && ((row_i + py*O_TILE_WIDTH) < numRows) && (col_i + px*O_TILE_WIDTH >= 0) && (col_i + px*O_TILE_WIDTH < numCols))
				{
					Ns[ty + py*O_TILE_WIDTH][tx + px*O_TILE_WIDTH][0] = InputImage[((row_i + py*O_TILE_WIDTH)*numCols + col_i + px*O_TILE_WIDTH) * 3 + 0];
					Ns[ty + py*O_TILE_WIDTH][tx + px*O_TILE_WIDTH][1] = InputImage[((row_i + py*O_TILE_WIDTH)*numCols + col_i + px*O_TILE_WIDTH) * 3 + 1];
					Ns[ty + py*O_TILE_WIDTH][tx + px*O_TILE_WIDTH][2] = InputImage[((row_i + py*O_TILE_WIDTH)*numCols + col_i + px*O_TILE_WIDTH) * 3 + 2];
				}
				else
				{
					Ns[ty + py*O_TILE_WIDTH][tx + px*O_TILE_WIDTH][0] = 0.0f;
					Ns[ty + py*O_TILE_WIDTH][tx + px*O_TILE_WIDTH][1] = 0.0f;
					Ns[ty + py*O_TILE_WIDTH][tx + px*O_TILE_WIDTH][2] = 0.0f;
				}
			}
		}
	}
...
int main(void)
{
	...
	imshow("CPUImage", image); //Display the image in the window
