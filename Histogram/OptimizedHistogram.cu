...
__global__ void histogram_kernel(unsigned int *input, unsigned int *bins, unsigned int num_elements, unsigned int num_bins)
{
	...
	__shared__ unsigned int ds_bins[NUM_BINS];
	int copyphases = (NUM_BINS - 1) / BLOCK_SIZE + 1;
	for (int j = 0; j < copyphases;j++)
		if(threadIdx.x+j*BLOCK_SIZE<NUM_BINS)
			ds_bins[threadIdx.x+j*BLOCK_SIZE] = 0;
	
	__syncthreads();	//wait for this thread to finish 

	if (i < num_elements)
	{
		atomicAdd(&ds_bins[input[i]], 1);
	}
	...
	for (int j = 0; j < copyphases; j++)
		if (threadIdx.x + j*BLOCK_SIZE<NUM_BINS)
			atomicAdd(&bins[threadIdx.x + j*BLOCK_SIZE], ds_bins[threadIdx.x + j*BLOCK_SIZE]);
