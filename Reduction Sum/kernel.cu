...
void totalCpu(float *input, float *sum, int len)
{
...
int main(void)
{
	...
	cudaError_t err;
	deviceInput = NULL;
	err = cudaMalloc((void **)&deviceInput, sizeof(float) * inputLength);
	if (err != cudaSuccess)
	{
	...
	deviceOutput = NULL;
	err = cudaMalloc((void **)&deviceOutput, sizeof(float)* outputLength);
	if (err != cudaSuccess)
	{
	...
	********************************************************************/
	...
	totalCpu(hostOutput, &Sum, outputLength);
	clock_t end = clock();
	...
	totalCpu(hostInput,& Sum_CPU, inputLength);
	clock_t end2 = clock();
	double time_spent2 = (double)(end2 - begin2) / CLOCKS_PER_SEC * 1000;
